...
const onDelete = (id: number) => {
  axios
    .delete(backendUrl + "/api/v1/public/jobs/link", {
      headers: {
...
const onVisit = (id: number) => {
  axios
    .put(
      backendUrl + "/api/v1/public/jobs/view",
      {
        id,
      },
      {
...
export function JobCard(link: JobLink) {
  const { details } = useProfile();
  return (
    <div className="rounded-lg border my-3  bg-gradient-to-r from-blue-300 to-violet-400 backdrop-blur-sm p-2 hover:shadow-2xl cursor-pointer hover:-translate-y-1 transi shadow-xl">
      <div className="bg-gradient-to-tr from-white/25 via-white/65 to-white/25 backdrop-blur-lg shadow-lg rounded-lg border p-3">
        ...
        <div className="mt-2 space-y-2">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <span className="text-sm text-gray-700">Role: {link.role}</span>
          </div>
        </div>
